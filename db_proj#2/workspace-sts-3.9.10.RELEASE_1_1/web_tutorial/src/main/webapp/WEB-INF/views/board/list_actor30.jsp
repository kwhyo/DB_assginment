...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
   ...
   <div id="front-cafe" style="cursor:hand;cursor:pointer;background-image:none;width:1080px;max-height: 340px">
              <a name="myCafeUrlLink" onclick="updateMoveMenu()" href="homepage" target="cafe_main">
               <img src="https://cafefiles.pstatic.net/MjAxODEwMTVfMTI3/MDAxNTM5NTg1MTEwMTUw.1apXRaFFas_uvUywfp783OYAQN1Zyi2wSr3Rbxdy8Lcg.lcbXm-R8x0MO3gQD4tz-mN_R-6HS_GXO7-usJ0xkUS4g.PNG.kioeiso/1080x129_cafe_main.png" alt="중고월드" onload="resizeTopSkin(this)" class="png24" width="1080">
              </a>
           </div>
    </div><!-- /.col-lg-12-->
    </div><!-- /.row -->
    ...
    <c:forEach var="boardVO" items="${listActor30}">
    <tr>
       <td>${boardVO.a_ID}</td>
       <td><a href="${path}/board/read_actor${pageMaker.makeSearch(pageMaker.criteria.page)}&a_name=${boardVO.a_name}">${boardVO.a_name}</a></td>
       <td>${boardVO.a_birthday}</td>
       <td>${boardVO.a_gender}</td>
    </tr>
          </c:forEach>
       ...
       <div class="text-center">
          <ul class="pagination">
             <c:if test="${pageMaker.prev}">
                   <li>
                      <a href="${path}/board/list_actor30${pageMaker.makeSearch(pageMaker.startPage - 1)}">&laquo;</a>
                   </li>
                       </c:if>
                          <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
                          <li <c:out value="${pageMaker.criteria.page == idx? 'class=active':''}"/>>
                             <a href="${path}/board/list_actor30${pageMaker.makeSearch(idx)}">${idx}</a>
                           </li>
                         </c:forEach>
                  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                      <li>
                        <a href="${path}/board/list_actor30${pageMaker.makeSearch(pageMaker.endPage + 1)}">&raquo;</a>
                       </li>
                 </c:if>
          </ul>
       </div>
       ...
       if(result == 'SUCCESS'){
        ...
        $(document).ready(function () {
               ...
               $("#searchBtn").on("click", function (event) {
                   self.location = "list_actor30${pageMaker.makeQuery(1)}"
