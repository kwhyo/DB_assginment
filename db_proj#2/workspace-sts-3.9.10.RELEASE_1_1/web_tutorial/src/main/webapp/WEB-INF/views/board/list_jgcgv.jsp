...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
   ...
   <div id="front-cafe" style="cursor:hand;cursor:pointer;background-image:none;width:1080px;max-height: 340px">
    ...
    <c:forEach var="boardVO" items="${listJGcgv}">
    <tr>
       <td>${boardVO.t_name}</td>
       ...
       <div class="text-center">
          <ul class="pagination">
             <c:if test="${pageMaker.prev}">
                   <li>
                      <a href="${path}/board/list_jgcgv${pageMaker.makeSearch(pageMaker.startPage - 1)}">&laquo;</a>
                   </li>
                       </c:if>
                          <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
                          <li <c:out value="${pageMaker.criteria.page == idx? 'class=active':''}"/>>
                             <a href="${path}/board/list_jgcgv${pageMaker.makeSearch(idx)}">${idx}</a>
                           </li>
                         </c:forEach>
                  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
                      <li>
                        <a href="${path}/board/list_jgcgv${pageMaker.makeSearch(pageMaker.endPage + 1)}">&raquo;</a>
                       </li>
                 </c:if>
          </ul>
       </div>
       ...
       if(result == 'SUCCESS'){
        ...
        $(document).ready(function () {
               ...
               $("#searchBtn").on("click", function (event) {
                   self.location = "list_jgcgv${pageMaker.makeQuery(1)}"
